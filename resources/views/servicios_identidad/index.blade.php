...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{!! trans('messages.servicioidentidad_nacional') !!}</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="<?php echo url('/'); ?>">{!! trans('messages.inicio') !!}</a></li>
                    <li class="breadcrumb-item active">{!! trans('messages.servicioidentidad_nacional') !!}</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
        ...
        <div class="card-header">
            <div class="card-tools">
                    <?php if (auth()->user()->rol == 'A' ||  auth()->user()->rol == 'E') { ?>
                        <button class="btn btn-sm btn-danger" style="background-color: #ffffff;color: #dc3545;font-weight: bold;border: 1px solid #dc3545;" onclick="location.href='<?php echo url('/papelera_identidad_nacional'); ?>'"><i class="fa fa-trash"></i> {!! trans('messages.papelera') !!}({{count($trash1)}})</button>
                    <?php  } ?>
              ...
              <div class="card bg-light d-flex flex-fill">
                ...
                <div class="card-footer">
                  <div class="text-right">
                    <a href="{{ url('detalles_identidad_nacional', ['id' => $key->id]) }}" class="btn btn-sm btn-info"><i class="fas fa-file-alt"></i> {!! trans('messages.detalle') !!}</a>
                ...
                <?php else : ?>
                  <div class=" pt-40 pb-30"> 
                    <div class="alert alert-warning" role="alert">
                     <i class="fas fa-exclamation-triangle"></i> {!! trans('messages.alerta_identidad') !!}
                    </div>
                  </div> 
                <?php endif; ?> 
          </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">{!! trans('messages.servicioidentidad_nacional') !!}</h4>
...
<script>
    ...
    function formulario(id = null) {
        $.ajax({
            url: '<?php echo url('/servicioidentidad') ?>',
            type: 'POST',
            data: {
                id: id
            },
    ...
    function eliminar(id = null) {
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?php echo url('/eliminar_servicio_identidad') ?>',
                    type: 'POST',
                    data: {
                        id: id
                    },
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?php echo url('/deshabilitarservicioidentidad') ?>',
                    type: 'POST',
                    data: {
                        id: id
                    },
