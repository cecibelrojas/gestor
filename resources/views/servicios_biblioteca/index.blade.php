...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">{!! trans('messages.serviciobiblioteca') !!}</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="<?php echo url('/'); ?>">{!! trans('messages.inicio') !!}</a></li>
                    <li class="breadcrumb-item active">{!! trans('messages.serviciobiblioteca') !!}</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
        ...
        <div class="card-header">
            <div class="card-tools">
                    <?php if (auth()->user()->rol == 'A' ||  auth()->user()->rol == 'E') { ?>
                        <button class="btn btn-sm btn-danger" style="background-color: #ffffff;color: #dc3545;font-weight: bold;border: 1px solid #dc3545;" onclick="location.href='<?php echo url('/papelera_biblioteca'); ?>'"><i class="fa fa-trash"></i> {!! trans('messages.papelera') !!}({{count($trash1)}})</button>
                    <?php  } ?>
                ...
                <?php else : ?>
                  <div class=" pt-40 pb-30"> 
                    <div class="alert alert-warning" role="alert">
                     <i class="fas fa-exclamation-triangle"></i> {!! trans('messages.alerta_biblioteca') !!}
                    </div>
                  </div> 
                <?php endif; ?> 
          </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">{!! trans('messages.serviciobiblioteca') !!}</h4>
...
<script>
    ...
    function formulario(id = null) {
        $.ajax({
            url: '<?php echo url('/serviciobiblioteca') ?>',
            type: 'POST',
            data: {
                id: id
            },
    ...
    function eliminar(id = null) {
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?php echo url('/eliminar_servicio_biblioteca') ?>',
                    type: 'POST',
                    data: {
                        id: id
                    },
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?php echo url('/deshabilitarserviciobiblioteca') ?>',
                    type: 'POST',
                    data: {
                        id: id
                    },
