...
<div class="row">
    ...
    <img onclick="$('#banner').trigger('click');" src="<?php echo ($data && !empty($data['banner'])) ? url('/') . $data['banner'] : asset('archivos/banner_biblioteca/img.png'); ?>" style="object-fit: cover;box-shadow: 1px 1px 8px 0px #7e7e7e;cursor: pointer; width: 100%" class="imgpreview">
...
<script>
    ...
    $('#btnSave').click(function() {
        ...
        $.ajax({
            url: '<?php echo url('/guardar-bnnr_serviciobiblioteca') ?>',
            type: 'POST',
            cache: false,
            contentType: false,
            processData: false,
