...
<div class="row">
    ...
    <div class="col-lg-8">
        ...
        <select class="form-control input-sm" id="tipo" style="border: 1px solid #b9b9b9;">
            <option value="">Seleccionar</option>
            <?php if (count($tipos) > 0) : ?>
                <?php foreach ($tipos as $key) : ?>
                    <option <?php echo ($data && $data['tipo'] == $key['item']) ? "selected" : ""; ?> value="<?php echo $key['item']; ?>"><?php echo $key['nombre']; ?></option>
                <?php endforeach; ?>
            <?php endif; ?>
        </select>
    </div>
    <div class="col-lg-4">
        ...
        <input type="number" class="form-control input-sm" value="<?php echo $data ? $data['orden'] : ''; ?>" id="orden" style="border: 1px solid #b9b9b9;">
    </div>
...
<script>
    $('#btnSave').click(function() {
        ...
        $.ajax({
            url: '<?php echo url('/guardar-widget') ?>',
            type: 'POST',
            data: {
                id: $('#id').val(),
                nombre: $('#nombre').val(),
